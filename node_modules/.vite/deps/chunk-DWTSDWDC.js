...
var Engine = class {
  ...
  async load(params) {
    ...
    const { Container } = await import("./Container-ZD2Z5RFC.js"), id = params.id ?? (domSourceElement == null ? void 0 : domSourceElement.id) ?? `tsparticles${Math.floor(getRandom() * loadRandomFactor).toString()}`, { index, url } = params, options = url ? await getDataFromUrl({ fallback: params.options, url, index }) : params.options, currentOptions = itemFromSingleOrMultiple(options, index), { items } = this, oldIndex = items.findIndex((v) => v.id.description === id), newItem = new Container({
...
function initEngine() {
  ...
  if (existing == null ? void 0 : existing.pluginManager) {
    return existing;
  }
  return new Engine();
}
...
//# sourceMappingURL=chunk-DWTSDWDC.js.map
